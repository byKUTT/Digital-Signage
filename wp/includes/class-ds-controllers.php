...
/**
 * Controller/domain model for one Linux PC driving many displays.
 */
class DS_Controllers {
	...
	public static function create_request( $platform, $hostname, array $outputs, $legacy_screen_token = '' ) {
		...
		return array(
			...
			'token'       => $token,
			'code'        => $code,
			'pairing_url' => home_url( '/c/' . $public_id . '/main/' ),
		);
	}
	...
	public static function get_assignments( $controller_id ) {
		...
		foreach ( $rows as $row ) {
			$out[] = array(
				...
				'pairing_url'  => home_url( '/c/' . self::get_public_id( $controller_id ) . '/' . rawurlencode( $row->output_key ) . '/' ),
			);
		}
		return $out;
	}
	...
	public static function delete( $controller_id ) {
		...
		delete_option( 'ds_music_controller_' . $controller_id );
		delete_option( 'ds_spotify_output_' . $controller_id );
	}
}
