...
class DS_CPT {
	...
	public function register_rewrite_rules() {
		...
		add_rewrite_rule( '^play/([^/]+)/?$', 'index.php?ds_screen_token=$matches[1]', 'top' );
		...
		add_rewrite_rule( '^tv/?$', 'index.php?ds_tv_launcher=1', 'top' );
		...
		add_rewrite_rule( '^c/([a-f0-9-]{36})/([A-Za-z0-9_-]{1,100})/?$', 'index.php?ds_controller_id=$matches[1]&ds_controller_output=$matches[2]', 'top' );
		...
		add_rewrite_rule( '^preview/([0-9]+)/?$', 'index.php?ds_preview_channel=$matches[1]', 'top' );
	...
	public function register_post_types() {
		...
		register_post_type( 'ds_music_playlist', array_merge( $common, array( 'label' => __( 'Music playlists', 'digital-signage' ) ) ) );
	}
