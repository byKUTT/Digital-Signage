...
class DS_Spotify {
	...
	const CONTROLLER_OUTPUT_PREFIX = 'ds_spotify_output_';
	...
	public static function controller_output_key( $controller_id ) {
		return self::CONTROLLER_OUTPUT_PREFIX . absint( $controller_id );
	}

	public static function controller_player_config( $screen_id ) {
		$screen_id = absint( $screen_id );
		$controller_id = absint( get_post_meta( $screen_id, 'ds_controller_id', true ) );
		if ( ! $controller_id ) { return null; }
		$primary = null;
		foreach ( DS_Controllers::get_displays( $controller_id ) as $display ) {
			if ( (int) $display->screen_id === $screen_id ) { $primary = $display; break; }
		}
		if ( ! $primary || empty( $primary->is_primary ) ) { return null; }
		$session = (array) get_option( self::controller_output_key( $controller_id ), array() );
		$user_id = absint( $session['user_id'] ?? 0 );
		if ( ! $user_id || empty( $session['enabled'] ) ) { return null; }
		$token = self::instance()->access_token( $controller_id, $user_id );
		if ( is_wp_error( $token ) ) { return null; }
		$controller = DS_Controllers::get( $controller_id );
		return array(
			'enabled' => true,
			'access_token' => $token,
			'name' => sprintf( 'Screens byKUTT — %s', sanitize_text_field( $controller->name ?: $controller->hostname ) ),
			'revision' => sanitize_text_field( $session['revision'] ?? '' ),
		);
	}

	public static function controller_output_revision( $screen_id ) {
		$controller_id = absint( get_post_meta( absint( $screen_id ), 'ds_controller_id', true ) );
		$session = $controller_id ? (array) get_option( self::controller_output_key( $controller_id ), array() ) : array();
		return ! empty( $session['enabled'] ) ? sanitize_text_field( $session['revision'] ?? 'enabled' ) : 'off';
	}
	...
	public function ajax_control() {
		...
		elseif ( 'controller_output' === $operation ) {
			$enabled = ! empty( $_POST['enabled'] ) && 'false' !== $_POST['enabled'];
			update_option( self::controller_output_key( $controller_id ), array( 'enabled' => $enabled, 'user_id' => get_current_user_id(), 'revision' => wp_generate_uuid4() ), false );
			$result = array( 'enabled' => $enabled );
		}
