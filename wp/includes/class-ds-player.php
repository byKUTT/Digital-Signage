...
class DS_Player {
	...
	public function maybe_render_player() {
		...
		if ( ! $preview_channel && preg_match( '#^(?:signage/)?preview/([0-9]+)$#', $path, $matches ) ) { $preview_channel = $matches[1]; }
		if ( $preview_channel ) {
		...
		if ( ! $token && preg_match( '#^(?:signage/)?play/([A-Za-z0-9]+)$#', $path, $matches ) ) { $token = $matches[1]; }
		if ( ! $token ) {
			return;
		}
	...
	private function is_tv_launcher_request() {
		...
		return in_array( $this->get_relative_request_path(), array( 'tv', 'signage/tv' ), true );
	}
	...
	private function get_controller_request() {
		...
		$path = $this->get_relative_request_path();
		if ( preg_match( '#^(?:signage/controller|c)/([a-f0-9-]{36})/([A-Za-z0-9_-]{1,100})$#', $path, $matches ) ) {
	...
	public static function get_player_url( $screen ) {
		...
		return $token ? home_url( '/play/' . $token . '/' ) : '';
	}
	 ...
	 * token player so TV, Ubuntu, and Pi screens share one renderer.
	 */
	private function render_tv_launcher() {
		...
		$player_base      = esc_url_raw( home_url( '/play/' ) );
		$site_name        = get_bloginfo( 'name' );
