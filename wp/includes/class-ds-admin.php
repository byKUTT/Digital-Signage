...
class DS_Admin {
	...
	public function render_screen_edit() {
		...
		$long_player_url = $token ? home_url( '/play/' . $token . '/' ) : '';
		$channels    = get_posts( array( 'post_type' => 'ds_channel', 'posts_per_page' => -1 ) );
	...
	/**
	 * Queues a command for the on-device agent (pi/ds-agent) —
			...
			case 'wifi':
				...
				DS_CRUD::queue_device_command(
					...
					array(
						...
						// see pi/ds-agent, which discards it immediately after nmcli.
