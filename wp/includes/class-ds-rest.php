 ...
 *                                  hardware running the ds-agent (see pi/ds-agent)
...
class DS_REST {
	...
	public function get_changes( WP_REST_Request $request ) {
		...
		$response   = rest_ensure_response(
			array(
				...
				'spotify_revision' => DS_Spotify::controller_output_revision( $screen->ID ),
	...
	public function get_playlist( WP_REST_Request $request ) {
		...
		$response = rest_ensure_response(
			array(
				...
				'spotify'        => DS_Spotify::controller_player_config( $screen->ID ),
				'layout'         => $layout,
				'zone_bg'        => $zone_bg ?: '',
				'zones'          => (object) $zones,
				'defaults'       => $settings,
	...
	public function post_heartbeat( WP_REST_Request $request ) {
		...
		// telemetry — see pi/ds-agent) hits this same endpoint but
