document.addEventListener('DOMContentLoaded', () => {
	...
	const elements = {
		...
		controllerOutput: app.querySelector('[data-controller-output]'),
	...
	const loadDevices = async () => {
		...
		const allDevices = sdkDeviceId ? [{ id: sdkDeviceId, name: 'This browser', type: 'Web Playback SDK' }, ...remoteDevices.filter((device) => device.id !== sdkDeviceId)] : remoteDevices;
	...
	elements.controllerOutput?.addEventListener('click', async () => {
		try {
			await call('controller_output', { enabled: 'true' });
			message('Controller speakers are starting. They will appear in the output list shortly.');
			elements.controllerOutput.disabled = true;
			setTimeout(async () => { try { await loadDevices(); } finally { elements.controllerOutput.disabled = false; } }, 8000);
		} catch (error) { message(error.message, true); }
	});
