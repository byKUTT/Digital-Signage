...
* **Fullscreen player** — a chrome-less, auto-fullscreen frontend at `/play/{token}/` that polls for updates, preloads the next slide, keeps playing from a local cache when offline, and supports multi-zone layouts (main + ticker + corner clock, split screen, grid).
...
The optional Spotify panel connects to Spotify Accounts (`accounts.spotify.com`), the Spotify Web API (`api.spotify.com`), and the Web Playback SDK (`sdk.scdn.co`) for OAuth, search, user-triggered controls, and an explicitly enabled controller-speaker output. Enabling it sends the connected account and playback requests to Spotify under Spotify's Developer Terms and Privacy Policy. It is disabled until a site administrator supplies application credentials and a user connects their own account. Audible signage video temporarily lowers controller-local SDK playback over 1.5 seconds. Spotify Premium is required; the site operator remains responsible for Spotify account, venue, and public-performance licensing terms.
...
* Added short `/screens/`, `/login/`, `/register/`, `/tv/`, `/play/`, `/preview/`, and `/c/` routes while preserving legacy URLs.
* Fixed single-video looping, failed-video recovery, stable play-to-end rotation, and 1.5-second ducking for both local music and controller Spotify playback.
* Restored the site-admin Screens settings area, added a direct Spotify setup link, and made the primary Ubuntu output available as a Web Playback SDK device.
* Rebuilt all 32 templates as image-led editable Vellum documents with exact landscape and portrait preview files generated from the same source.
* Reworked pairing and waiting displays around the responsive Screens byKUTT identity and simplified the portal into clear one-click navigation groups.
* Fixed deleted controllers getting stuck with rejected credentials; Ubuntu now requests a fresh pairing code without losing its URL, profiles, or settings.
...
= 4.0.0 =
* Added the authenticated `/screens/` frontend portal with a simplified overview, screens, channels, controllers, and people experience.
...
= 2.10.1 =
* Fixed `/tv/` showing the WordPress 404 page when a host did not flush permalink rewrite rules after updating the plugin. The TV launcher now also recognizes its request path directly.

= 2.10.0 =
* Added a private VIDAA 9 smart-TV launcher at `/tv/` with persistent device identity, rotating six-character pairing, automatic reconnect and a long-press OK reset.
...
* New `pi/setup-portal/`: a device with no configuration yet puts up a temporary WiFi hotspot with a one-page setup form (WiFi + site URL) instead of the kiosk — connect from a phone, submit, and it provisions itself and reboots into its pairing screen.
