...
class ControllerRegistrationLost(RuntimeError):
    """The server no longer recognises this device identity."""
...
class Controller:
    ...
    def request(
        ...
        try:
            ...
            is_identity_request = authenticated and "/controller/" in path and path.endswith(
                ("/heartbeat", "/status")
            )
            if is_identity_request and error.code in (401, 404):
                self.forget_identity()
                raise ControllerRegistrationLost(
                    "controller was removed from the server; requesting a new pairing code"
                ) from error
            raise
        if not isinstance(data, dict):
    ...
    def forget_identity(self) -> None:
        """Drop only server credentials, keeping the site and kiosk settings intact."""
        self.identity = {}
        try:
            self.identity_path.unlink(missing_ok=True)
        except OSError as error:
            self.log(f"Could not remove stale controller identity: {error}")
        self.force_refresh = True
        ...
        try:
            while not self.stop_requested:
                try:
                    ...
                    consecutive_errors = 0
                    self.last_error = ""
                except ControllerRegistrationLost as error:
                    self.stop_players()
                    assignments = []
                    schedule = None
                    last_heartbeat = 0.0
