...
if [ ! -r /etc/os-release ] || ! ( . /etc/os-release; [ "${ID:-}" = "ubuntu" ] ); then
	echo "This installer supports Ubuntu Desktop. Use the pi installer on Raspberry Pi OS." >&2
	exit 1
fi
...
copy_managed_file "$managed_repo/ubuntu/ds_ubuntu_controller.py" "$install_root/ds_ubuntu_controller.py" 755
copy_managed_file "$managed_repo/ubuntu/ds-ubuntu-autostart.sh" /usr/local/bin/ds-ubuntu-autostart 755
copy_managed_file "$managed_repo/ubuntu/update-kiosk.sh" /usr/local/sbin/digital-signage-update 755
copy_managed_file "$managed_repo/ubuntu/digital-signage-root-command" /usr/local/sbin/digital-signage-root-command 755
copy_managed_file "$managed_repo/ubuntu/systemd/digital-signage-ubuntu-update.service" /etc/systemd/system/digital-signage-ubuntu-update.service 644
...
copy_managed_file "$managed_repo/ubuntu/autostart/bykutt-digital-signage.desktop" /etc/xdg/autostart/bykutt-digital-signage.desktop 644
...
python3 "$managed_repo/ubuntu/configure_gdm.py" "$gdm_config" "$kiosk_user"
