...
copy just the `pi` folder from your computer instead, e.g.
with `scp` from the machine that has it:
```bash
scp -r pi pi@<hostname-or-ip>:~/
...
> `ds-agent/ds-agent.py` isn't next to it — grab the full `pi/`
